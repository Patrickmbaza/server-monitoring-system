                ...
                if notifier and status_changes:
                    for change in status_changes:
                        if change["new_status"] == "down":
                            ...
                            notifier.send_downtime_alerts(
                                change["host"], change["timestamp"]
                            )
                        elif (
                            change["new_status"] == "up"
                            and change["previous_status"] == "down"
                        ):
                            logger.info(f"Host {change['host']['name']} is back online")
                            notifier.send_resolved_alerts(
                                change["host"], change["timestamp"]
                            )
...
def test_alert(alert_type):
    ...
    if alert_type == "downtime":
        ...
        logger.info(
            f"Testing downtime alert at {format_datetime_for_api(current_time)}"
        )
        ...
        current_time = datetime.now()
        logger.info(
            f"Testing resolved alert at {format_datetime_for_api(current_time)}"
        )
