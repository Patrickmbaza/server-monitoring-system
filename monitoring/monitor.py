...
class HostMonitor:
    ...
    def load_config(self):
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded {len(self.hosts)} hosts from configuration"
            )
    ...
    def validate_host_config(self, host: Dict[str, Any], index: int) -> bool:
        ...
        if host["check_type"] not in ["tcp", "http", "https"]:
            logger.error(
                f"Host '{host['name']}' has invalid check_type: '{host['check_type']}'. Must be 'tcp', 'http', or 'https'"
            )
            return False
        ...
        try:
            ...
            if not (1 <= port <= 65535):
                logger.error(
                    f"Host '{host['name']}' has invalid port: {port}. Must be between 1-65535"
                )
                return False
        except ValueError:
            logger.error(
                f"Host '{host['name']}' has invalid port: {host['port']}. Must be a number"
            )
            return False
        ...
        if len(ip_parts) != 4:
            logger.warning(
                f"Host '{host['name']}' has non-standard IP address: {host['ip_address']}"
            )
    ...
    def check_host(self, host: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if check_type == "tcp":
                ...
                is_up = self.check_http_service(
                    host["ip_address"], host["port"], check_type == "https"
                )
            else:
                logger.error(
                    f"Unknown check type for host {host['name']}: {check_type}"
                )
                is_up = False
    ...
    def check_http_service(
        self, ip: str, port: int, use_https: bool = False, timeout: int = 10
    ) -> bool:
        """
        ...
        try:
            ...
            headers = {"User-Agent": "Server-Monitoring-System/1.0"}
            ...
            if is_up:
                logger.debug(
                    f"HTTP check successful for {url}, status: {response.status_code}"
                )
            else:
                logger.warning(
                    f"HTTP check failed for {url}, status: {response.status_code}"
                )
        ...
        for host in self.hosts:
            try:
                ...
                if previous_status != new_status:
                    ...
                    status_changes.append(
                        {
                            ...
                            "downtime_duration": (
                                self.get_downtime_duration(host_key)
                                if new_status == "up"
                                else None
                            ),
                        }
                    ...
                    if (
                        self.host_status[host_key]["check_count"] % 10 == 0
                    ):  # Every 10th check
                        logger.debug(
                            f"Status unchanged: {host['name']} ({host['ip_address']}) is {new_status}"
                        )

            except Exception as e:
                logger.error(
                    f"Error monitoring host {host.get('name', 'unknown')}: {str(e)}"
                )
                continue

        logger.info(
            f"Monitoring cycle completed. Status changes detected: {len(status_changes)}"
        )
        return status_changes
    ...
    def add_host(self, host_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for existing_host in self.hosts:
                if (
                    existing_host["ip_address"] == host_config["ip_address"]
                    and existing_host["port"] == host_config["port"]
                ):
                    logger.warning(
                        f"Host with IP {host_config['ip_address']} and port {host_config['port']} already exists"
                    )
                    return False
